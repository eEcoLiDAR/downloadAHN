...
"""
Downloader for AHN3 dataset. Used with eEcoLiDAR project
...
def argument_parser():
    parser = argparse.ArgumentParser(description="""This script checks compares a local repository of AHN3 .LAZ point cloud data fileas against
    the online AHN3 repository and downloads any new or updated files. Can be run on a single machine or in distributrd fashion across multiple VMs.
    ...
    parser.add_argument('-v', '--numbervms', default=1, help='number of VMs being used', type=int)
    parser.add_argument('-j','--vmj', default=0,help='numerical id of VM instance; default 0',type=int)
    parser.add_argument('-p','--proc',default=1,help='number of processes [per VM]; default is 1',type=int)
    parser.add_argument('-i','--inputlist',default=None,help='optional input list')
    parser.add_argument('-c','--copylocal',default=False,help='Boolean flag. if true up-to-date files from local repository will be copied to output directory',type=bool)
    return parser
...
def build_url(top10nlMapTileFileName):
    url = 'https://geodata.nationaalgeoregister.nl/ahn3/extract/ahn3_laz/'+top10nlMapTileFileName
    return url


def check_local(top10nlMapTile,localFilesPath):


    mapTileFileName = build_filename(top10nlMapTile)
...
def check_remote(top10nlMapTile):

    mapTileFileName = build_filename(top10nlMapTile)
    mapTileFileUrl = build_url(mapTileFileName)
...
def copy_execute(top10nlMapTile,localFilesPath,outputDirectory):
...
def download_decider(top10nlMapTile, localFilesPath):

    mapTileFileExistsLocal, localSize, localDate = check_local(top10nlMapTile,localFilesPath)

    mapTileFileExistsRemote, remoteSize, remoteDate = check_remote(top10nlMapTile)
...
def download_execute(top10nlMapTile,outputDir):
...
def maptile_downloader(top10nlMapTile,localFilesPath,outputDir,copylocal):

    executeDownload, localExists = download_decider(top10nlMapTile,localFilesPath)
    ...
    if executeDownload == True:
        ...
        downloadSuccess = download_execute(top10nlMapTile,outputDir)
        ...
        if localExists == True:
            ...
            if copylocal == False:
                ...
                if localFilesPath == outputDir:
                    ...
                    copySuccess = copy_execute(top10nlMapTile,localFilesPath,outputDir)
        else:
...
def run(localFilesPath,outputDir,numberProcs,vmid,inputList=[],copylocal=False):
    #check input
    ...
    for i in range(numberProcs):
        processes.append(multiprocessing.Process(target=runTileDownloadProc,args=(i,tasksQueue,resultsQueue,localFilesPath,outputDir,copylocal)))
        processes[-1].start()
    ...
    legacyjs = '/downloadList_vm'+str(vmid)+'-'+str(now.timestamp())+'.js'
    currentjs = '/downloadList_vm'+str(vmid)+'-latest.js'
...
def runTileDownloadProc(processIndex,tasksQueue,resultsQueue,localFilesPath,outputDir,copylocal):
    kill_received = False
    while not kill_received:
        ...
        if mapTile == None:
            ...
            wasDownloaded, downloadSucceded, wasCopied, copySucceded = maptile_downloader(mapTile,localFilesPath,outputDir,copylocal)
...
def main():
    ...
    print('number of nodes/VMs being used: {0}. This is node/VM {1}.'.format(args.numbervms,args.vmj))
    print('running {} processes on this node/VM'.format(args.proc))
    ...
    if len(allMapTiles) == 0:
        print('no tiles to check. aborting')
    else:

        sublistsMapTiles = split_input_list(args.numbervms,allMapTiles)
        mapTileList = sublistsMapTiles[args.vmj]

        print(sublistsMapTiles)
        print('---')
        print(mapTileList)
        ...
        try:
            t0 = time.time()
            print('starting ...')
            run(args.localrepository, args.outputdirectory, args.proc, args.vmj, inputList=mapTileList,copylocal=args.copylocal)
